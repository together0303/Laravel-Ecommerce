...
class SellerProfileController extends Controller
{
    ...
    public function myShop(){
        $user = Auth::guard('api')->user();
        $seller = Vendor::with('socialLinks','products', 'user')->where('user_id',$user->id)->first();
        $setting = Setting::first();

        return response()->json(['user' => $user, 'seller' => $seller, 'setting' => $setting]);
    }
    ...
    public function emailHistory(){
        ...
        return response()->json(['emails' => $emails, 'user' => $user , 'setting' => $setting]);
